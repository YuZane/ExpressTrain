...
static dma_color_t ColorBuf[3][LED_NUM + 4];
u32 dma_idle = 1;


/* 呼吸灯曲线表 */
uint16_t index_heart[215] = {7, 7, 7, 7, 7, 7, 8, 8, 8, 8, 8, 8, 9, 9, 9, 9, 9, 10, 10, 10, 11, 11, 12, 12, 
                               ...
                               213, 221, 229, 238, 238, 240, 247, 247, 247, 240, 238, 238, 229, 221, 213, 205, 197, 190,183, 176, 170, 164, 158, 152, 146, 
                               ...
                               20, 19, 18, 18, 17, 16, 16, 15, 15, 14, 14, 13, 13, 12, 12, 11, 11, 10, 10, 10, 9, 9, 9, 9, 9, 8, 8, 8, 8, 8, 8, 7, 7, 7, 7, 7, 7};


uint16_t index_forward[116] = {3 ,3, 3, 3, 7, 7, 7, 7, 7, 7, 8, 8, 8, 8, 8, 8, 9, 9, 9, 9, 9, 10, 10, 10, 11, 11, 12, 12, 
                               13, 13, 14, 14, 15, 15, 16, 16, 17, 18, 18, 19, 20, 20, 21, 22, 23, 24, 25, 25, 26, 27, 28, 30, 31, 32, 33, 
                               34, 36, 37, 38, 40, 41, 43, 45, 46, 48, 50, 52, 54, 56, 58, 60, 62, 65, 67, 70, 72, 75, 78, 81, 84, 87, 90, 
                               94, 97, 101, 105, 109, 113, 117, 122, 126, 131, 136, 141, 146, 152, 158, 164, 170, 176, 183, 190, 197, 205, 
                               213, 221, 229, 238, 238, 240, 247, 247, 247};

uint16_t index_backward[116] =   {247, 247, 247, 240, 238, 238, 229, 221, 213, 205, 197, 190,183, 176, 170, 164, 158, 152, 146, 
                               141, 136, 131, 126, 122, 117, 113, 109, 105, 101, 97, 94, 90, 87, 84, 81, 78, 75, 72, 70, 67, 65, 62, 60, 58, 
                               56,  54,  52, 50, 48, 46, 45, 43, 41, 40, 38, 37, 36, 34, 33, 32, 31, 30, 28, 27, 26, 25, 25, 24, 23, 22, 21, 20, 
                               20,  19,  18, 18, 17, 16, 16, 15, 15, 14, 14, 13, 13, 12, 12, 11, 11, 10, 10, 10, 9, 9, 9, 9, 9, 8, 8, 8, 8, 8, 8,
                               7,   7,   7, 7, 7, 7, 3 ,3, 3, 3, 0, 0, 0};
...
void setOneColor_dma(dma_color_t *color, uint32_t rgb) {
    ...
    uint8_t j;
		for (j = 0; j < 8; j++) {
				color->R[j] = (r & (1 << (7 - j))) ? LED_HIGH : LED_LOW;
				color->G[j] = (g & (1 << (7 - j))) ? LED_HIGH : LED_LOW;
				color->B[j] = (b & (1 << (7 - j))) ? LED_HIGH : LED_LOW;
		}
}
...
{
    int i;
    for (i = 0; i < sizeof(index_heart) / sizeof(uint16_t); i++)
    {
        LED_LIGHT(rgb, index_heart[i]);
    }
}
...
void Forward(u32 rgb)
{
    int i;
    setAllColor_dma(ColorBuf[2], 0x000000);
    for (i = 0; i < sizeof(index_forward) / sizeof(uint16_t); i++)
    {
        LED_LIGHT(rgb, index_forward[i]);
    }
}
...
void Backward(u32 rgb)
{
    int i;
    setAllColor_dma(ColorBuf[2], 0x000000);
    for (i = 0; i < sizeof(index_backward) / sizeof(uint16_t); i++)
    {
        LED_LIGHT(rgb, index_backward[i]);
    }
}
